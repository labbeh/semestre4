...
public abstract class SpaceObject
{
	...
	{
		return ( dgr<MAX_DGR && dgr>MIN_DGR ? dgr : ( dgr>MAX_DGR ? dgr - MAX_DGR : - dgr - MAX_DGR ) );
	}
	
	private double verifierPosition(double val)
	{
		return ( val<1000 && val>0 ? val : ( val>1000 ? val - 1000 : - val - 1000 ) );
	}
	
	public void setX(double x)     { this.pos.setX(verifierPosition(x)); }
	public void setY(double y)     { this.pos.setY(verifierPosition(y)); }
	...
	public List<Coordonnees> getCoordsContour() { return this.traiterImage.getCoordsContour(); }
	
	public boolean appartient(Coordonnees pos)
	{
		List<Coordonnees> list = this.traiterImage.getCoordsContour();
